...
bool MS5611_I2C::begin(ms5611_osr_t osr)
{
    Wire.begin();
...
void MS5611_I2C::setOversampling(ms5611_osr_t osr)
{
    switch (osr)
    {
...
ms5611_osr_t MS5611_I2C::getOversampling(void)
{
    return (ms5611_osr_t)uosr;
}

void MS5611_I2C::reset(void)
{
...
uint32_t MS5611_I2C::readRawTemperature(void)
{
    Wire.beginTransmission(MS5611_ADDRESS);
...
uint32_t MS5611_I2C::readRawPressure(void)
{
    Wire.beginTransmission(MS5611_ADDRESS);
...
int32_t MS5611_I2C::readPressure(bool compensation)
{
...
double MS5611_I2C::readTemperature(bool compensation)
{
    uint32_t D2 = readRawTemperature();
    int32_t dT = D2 - (uint32_t)fc[4] * 256;
...
double MS5611_I2C::getAltitude(double pressure, double seaLevelPressure)
{
    return (44330.0f * (1.0f - pow((double)pressure / (double)seaLevelPressure, 0.1902949f)));
}

// Calculate sea level from Pressure given on specific altitude
double MS5611_I2C::getSeaLevel(double pressure, double altitude)
{
    return ((double)pressure / pow(1.0f - ((double)altitude / 44330.0f), 5.255f));
}

// Read 16-bit from register (oops MSB, LSB)
uint16_t MS5611_I2C::readRegister16(uint8_t reg)
{
    uint16_t value;
    Wire.beginTransmission(MS5611_ADDRESS);
    #if ARDUINO >= 100
...
uint32_t MS5611_I2C::readRegister24(uint8_t reg)
{
    uint32_t value;
    Wire.beginTransmission(MS5611_ADDRESS);
    #if ARDUINO >= 100
