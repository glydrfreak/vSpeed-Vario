...
/*
float RUNNING_AVERAGE::AVERAGE(float newData, int samplesPerSec){
  float averagingDuration = 0.25;   // (Default time in seconds to average)
  float filteredData = 0;
  float sum = 0;
  samplesToAverage = samplesPerSec * averagingDuration;
  //if(dbg){Serial.print(" "); Serial.print(samplesToAverage);}
  //if(dbg){Serial.print(" "); Serial.print(f1);}
    if(f1){
    f1 = false;  //only do this once...
